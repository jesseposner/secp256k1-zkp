...
void test_ecdsa_adaptor_spec_vectors_check(const unsigned char *adaptor_sig162, const unsigned char *msg32, const unsigned char *pubkey33, const unsigned char *encryption_key33, const unsigned char *decryption_key32, const unsigned char *signature64) {
    ...
    secp256k1_eckey_pubkey_parse(&encryption_key_ge, encryption_key33, 33);
    secp256k1_pubkey_save(&encryption_key, &encryption_key_ge);
    secp256k1_eckey_pubkey_parse(&pubkey_ge, pubkey33, 33);
    ...
    secp256k1_ecdsa_signature_serialize_compact(ctx, signature, &s);
    CHECK(secp256k1_memcmp_var(signature, signature64, 64) == 0);

    /* ecdsa_adaptor_recover */
    secp256k1_ecdsa_adaptor_extract_secret(ctx, adaptor_secret, &s, adaptor_sig162, &encryption_key);
    CHECK(secp256k1_memcmp_var(adaptor_secret, decryption_key32, 32) == 0);
}
